...
int main(int argc, char ** argv){
	...
	int dimension = 10;					// dimension of the vectorial space where the points are
	...
	for(int p = 1; p <= max_p; p += dimension / 5){
		...
		output << p << "\t" << (double)(learned_dichotomies) / N_iterations << "\t" << theory.getC(dimension, p) / pow(2, p) << endl;
	}
