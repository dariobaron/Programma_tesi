...
int main(int argc, char ** argv){
	...
	if( argc != 3 ){
		cerr << "USAGE: ./program.x mu sigma" << endl;
		return -1;
	}
	...
	int for_statistics = 50;		// repetition for each output point so that you can measure statistical error
	...
	Random< normal_distribution<>, double, double > overlap(randomEngine(), mu, sigma);
	...
	filename.pop_back();
	filename = "Output/" + filename + ".dat";
	cout << filename << endl;
	output.open(filename);

	//	creating the theoretical prevision
	Learning theory(dimension, max_p, psi);
	theory.compute();
		...
		for(int k = 0; k < for_statistics; ++k){
			...
			for(long iter_count = 0; iter_count < N_iterations; ++iter_count){
				...
				for(auto & i : points){
					...
					i.second = partner(i.first, overlap.generate(-0.999999, 1));
				}
